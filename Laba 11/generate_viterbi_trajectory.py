...
def Viterbi_algorithm(step):
    ...
    for i, x_c, y_c, h_c, w_c in zip(range(len(data_cur["x"])), data_cur["x"], data_cur["y"], data_cur["heading"], data_cur["weight"]):
...
if __name__ == "__main__":
    ...
    start = time.time()
    processes = [ multiprocessing.Process(target=Viterbi_algorithm) for step in range(1, total_steps) ]
    for p in processes:
        p.start()
    for p in processes:
        p.join()
